...
button.addEventListener("click", () => {
  document.write(`My favourite phone is: ${input.value}`);
  document.write("<br>");
  document.write(`\n Length of string is: ${input.value.length}`);
});
...
document.body.innerText = `String: 'Pakistani'
 Index of 'n': ${arrstr.indexOf("n")}`;
...
document.body.innerText = `String: 'Hello World'
 Last index of 'l': ${result}`;

// Question # 5

var string = "Pakistan";
document.body.innerText = `String: ${string} 
...
const button = document.querySelector("button");
button.addEventListener("click", () => {
  alert(`hello ${inputs[0].value.concat(" ").concat(inputs[1].value)}`);
});

// Question # 7

var str = "Hyderabad";
var result = str.replace("Hyder", "Islam");
...
document.body.innerHTML = `Value: ${strNum} <br>  Type: ${type} <br>  Value: ${num} <br>  Type: ${type1}`;
...
const button = document.querySelector("button");
button.addEventListener("click", () => {
  document.body.innerHTML = `User input: ${
    input.value
  } <br> Upper case: ${input.value.toUpperCase()}`;
});
// Question # 11
...
button.addEventListener("click", () => {
  ...
  document.body.innerHTML = `User input: ${input.value} <br> Title case: ${answer}`;
});
...
for (const items of A) {
  if (item.toLowerCase() == items) {
    document.body.innerHTML = `${item} is available at index ${A.indexOf(
      items
    )}`;
    temp++;
    break;
...
if (temp == 0) {
  document.body.innerHTML = `We are sorry, ${item} is not available in our bakery `;
}
...
document.body.innerHTML = `Text: ${str}<br>
...
button.addEventListener("click", () => {
  if (
    (Number.isInteger(Number(input.value.substring(1))) === false) &
    (parseFloat(input.value.substring(1)) > 0) &
...
const button = document.querySelector("button");
button.addEventListener("click", () => {
  var body = `The absolute value of ${input.value} is ${Math.abs(input.value)}`;
  document.body.innerHTML = body;
});
...
document.body.innerHTML = ` random dice value is ${randomIntFromInterval(
  1,
  6
)}`;
...
button.addEventListener("click", () => {
  ...
  if (x) {
    document.body.innerHTML = `${Math.floor(
      Math.random() * 2
    )} <br> Random coin value: Heads`;
  } else {
    document.body.innerHTML = `${Math.floor(
      Math.random() * 2
    )} <br> Random coin value: Tails`;
  }
});
// Question # 6
...
document.body.innerHTML = ` Random number between 1 and 100: ${randomIntFromInterval(
  1,
  100
)}`;
...
button.addEventListener("click", () => {
  if (
    input.value.match(regularExpression1) ||
    input.value.match(regularExpression2) ||
    input.value.match(regularExpression3) ||
    input.value.match(regularExpression4) ||
    input.value.match(regularExpression5)
  ) {
...
function secret_number(min, max) {
  // min and max included
  return Math.floor(Math.random() * (max - min + 1) + min);
}
...
document.body.innerText = `Current Month: ${months[d.getMonth()]}`;
...
if ((d.getDay() === 5) | (d.getDay() === 6)) {
  document.body.innerText = `It's Fun day`;
}
...
document.body.innerText = `Current date: ${d}
                           Elapsed milliseconds since January 1,1970 is ${Date.UTC(
                             2020,
                             6
                           )}
                           Elapsed minutes since January 1,1970 is ${
                             Date.UTC(2020, 6) / 60000
                           }`;
...
const diffDays = Math.round(
  Math.abs(
    (todayDate.getTime() - startingDateRamadan.getTime()) / oneDayMilliseconds
  )
);
...
function adding() {
  document.write(
    `the addition of two number is ${+inputs[0].value + +inputs[1].value}`
  );
}
...
button.addEventListener("click", () => {
  ...
  document.body.innerHTML = `string: ${
    input.value
  } <br> Titlecase string : ${answer.join(" ")}`;
});
...
var body = `
<input type='text' placeholder="Enter radius"></input>
<button>ok</button>
`;
...
button.addEventListener("click", () => {
  document.write(calcArea(input.value));
  document.write("<br>");
  document.write(calcCircumference(input.value));
});
